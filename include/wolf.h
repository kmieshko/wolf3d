...
typedef struct			s_wolf
{
	...
	t_double_constit	side_dist;
	t_double_constit	delta_dist;
	double				move_speed;
	double				rot_speed;
	t_constit			step;
	int					intersect;
...
void					ft_pixel_put(SDL_Surface *surface, int x, int y,
						int color);
void					find_person_position(t_wolf *wolf);
double					ft_clamp_with_param(double value, double min,
						double max);
...
void					fill_buffer_wall_colors(t_wolf *w,
						int buffer[WIN_H][WIN_W], int x);
void					get_music(Mix_Music *music);
void					draw_buffer(t_window *win, int buffer[WIN_H][WIN_W]);
void					draw_weapon(t_wolf *wolf, int buffer[WIN_H][WIN_W],
						int x);
void					get_floor_coord(t_wolf *wolf);
void					fill_buf_ceiling_floor(t_wolf *wolf,
						int buffer[WIN_H][WIN_W], int x);
